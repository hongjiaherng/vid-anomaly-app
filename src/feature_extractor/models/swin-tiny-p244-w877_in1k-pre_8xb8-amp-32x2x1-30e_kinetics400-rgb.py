...
import feature_extractor.transforms as my_transforms
...
def build_model() -> nn.Module:
    ...
    print(f"Swin (random): {calculate_mean_std_parameters(model)=}")
    checkpoint = torch.load(pretrained_path, map_location="cpu")["state_dict"]
    model.load_state_dict(checkpoint, strict=False)
    print(f"Swin (ckpt): {calculate_mean_std_parameters(model)=}")
    model.eval()
    return model
